...
func (r *Resolver) explodePackageRequires(pkgVar *Var) bf.Formula {
	...
	for _, req := range pkgVar.Package.Format.Requires.Entries {
		...
		if err != nil {
			...
			logrus.Warnf("Setting requirement of %s to NOT satisfied due to unresolvable dependency", pkgVar.Package.String())
...
func (r *Resolver) explodePackageConflicts(pkgVar *Var) bf.Formula {
	...
	for _, req := range pkgVar.Package.Format.Conflicts.Entries {
		...
		for _, s := range conflicts {
			...
			logrus.Infof("Conflict detected: %s conflicts with %s", s.Package.String(), pkgVar.Package.String())
...
func (res *Resolver) exportCNF(filename string) error {
	...
	logrus.Info("已将 SAT 问题导出为 problem.cnf 文件")
	return nil
}
